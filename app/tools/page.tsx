                    ...
                    <div className="tools-grid">
                        {[
                            ...
                            <div key={tool.id} className="tool-card" onClick={() => setActiveCalc(tool.id)}>
                                ...
                                <h3>{tool.title}</h3>
                                <p style={{ color: 'var(--text-secondary)', fontSize: '0.88rem' }}>{tool.desc}</p>
                            </div>
                        ))}
...
function CGPACalculator() {
    ...
    return (
        <div style={{ maxWidth: '700px', margin: '0 auto' }}>
            <h2 style={{ marginBottom: '20px' }}>📊 CGPA Calculator</h2>
            {rows.map((row, i) => (
                <div key={i} className="calc-row">
                    <input placeholder="Subject name" value={row.subject} onChange={e => updateRow(i, 'subject', e.target.value)} style={{ flex: 2 }} />
                    <select value={row.creditHours} onChange={e => updateRow(i, 'creditHours', Number(e.target.value))}>
                        {[1, 2, 3, 4].map(ch => <option key={ch} value={ch}>{ch} CH</option>)}
                    </select>
                    <select value={row.grade} onChange={e => updateRow(i, 'grade', e.target.value)}>
                        {Object.keys(gradePoints).map(g => <option key={g} value={g}>{g}</option>)}
                    </select>
                    <button className="btn btn-danger btn-sm" onClick={() => removeRow(i)}>✕</button>
                </div>
            ))}
            <button className="btn btn-secondary" onClick={addRow} style={{ marginTop: '12px' }}>+ Add Subject</button>
            <div className="calc-result">
                <div style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', marginBottom: '8px' }}>Your GPA</div>
                <div className="calc-result-number">{gpa}</div>
                <div style={{ marginTop: '8px', fontSize: '0.88rem', color: 'var(--text-muted)' }}>Total Credit Hours: {totalCredits}</div>
            </div>
        </div>
    );
}
...
function PercentageCalculator() {
    ...
    return (
        <div style={{ maxWidth: '500px', margin: '0 auto' }}>
            <h2 style={{ marginBottom: '20px' }}>📈 Percentage Calculator</h2>
    ...
    return (
        <div style={{ maxWidth: '600px', margin: '0 auto' }}>
            ...
            <div className="card" style={{ padding: '32px' }}>
                ...
                <h4 style={{ marginTop: '20px', marginBottom: '8px' }}>APA Format:</h4>
                <div style={{ padding: '12px', background: 'var(--bg-tertiary)', borderRadius: 'var(--radius-sm)', fontSize: '0.88rem', color: 'var(--text-secondary)' }}>{apa}</div>
                <h4 style={{ marginTop: '16px', marginBottom: '8px' }}>MLA Format:</h4>
                <div style={{ padding: '12px', background: 'var(--bg-tertiary)', borderRadius: 'var(--radius-sm)', fontSize: '0.88rem', color: 'var(--text-secondary)' }}>{mla}</div>
            </div>
        </div>
    );
}
